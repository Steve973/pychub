...
def fake_parser(monkeypatch):
    ...
    parser.add_argument("--show-compatibility", dest="show-compatibility", action="store_true")
...
# --- Coverage: show-compatibility path ---

def test_main_show_compatibility(monkeypatch, fake_bundle):
    """Test --show-compatibility flag."""
    root, libs = fake_bundle
    # Use the actual parser and argument name with dash
    monkeypatch.setattr(sys, "argv", ["pychub", "--show-compatibility"])

    compat_calls = {}

    def mock_show_compatibility(bundle_root):
        compat_calls["bundle_root"] = bundle_root

    monkeypatch.setattr(runtime_main, "show_compatibility", mock_show_compatibility)

    runtime_main.main()

    assert compat_calls["bundle_root"] == root
