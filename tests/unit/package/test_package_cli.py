...
def test_main_parses_analyze_compatibility(monkeypatch):
    """Test that --analyze-compatibility flag is parsed correctly."""
    from pychub.package import cli

    mock_process = Mock()
    monkeypatch.setattr(cli, "process_options", mock_process)
    monkeypatch.setattr("sys.argv", ["pychub", "pkg.whl", "--analyze-compatibility"])
...
def test_main_with_no_wheel_argument(monkeypatch):
    """Test that wheel is None when not provided."""
    from pychub.package import cli

    mock_process = Mock()
    monkeypatch.setattr(cli, "process_options", mock_process)
    monkeypatch.setattr("sys.argv", ["pychub", "--verbose"])
