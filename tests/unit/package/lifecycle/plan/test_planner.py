            ...
            with patch("pychub.package.lifecycle.plan.planner.copy_post_install_scripts") as mock_post:
                with patch("pychub.package.lifecycle.plan.planner.copy_included_files") as mock_inc:
                    with patch("pychub.package.lifecycle.plan.planner.copy_runtime_files") as mock_runtime:
                        result = plan_build(mock_buildplan, cache_dir, strategies)
    
    # Verify stage_wheels called correctly
    mock_stage.assert_called_once()
    args = mock_stage.call_args[0]
    assert args[0] == mock_buildplan
    assert isinstance(args[1], dict)
    assert args[2] == {"pkg1", "pkg2"}
    assert args[3] == strategies
    
    # Verify copy functions called with correct paths
    mock_pre.assert_called_once_with(mock_buildplan, cache_dir / "scripts" / "pre", ["setup.sh"])
    mock_post.assert_called_once_with(mock_buildplan, cache_dir / "scripts" / "post", ["cleanup.sh"])
    mock_inc.assert_called_once_with(mock_buildplan, cache_dir / "includes", ["config.yaml"])
    mock_runtime.assert_called_once_with(mock_buildplan, cache_dir / "runtime")
