...
from pychub.package import build_options_processor
...
def test_parse_chubproject_file_not_found():
    ...
    with pytest.raises(FileNotFoundError) as exc_info:
        build_options_processor.parse_chubproject(missing_path)
...
def test_parse_chubproject_success(monkeypatch, tmp_path):
    ...
    config_file = tmp_path / "valid_chubproject.toml"
    ...
    monkeypatch.setattr(ChubProject, "load_file", mock_load)

    result = build_options_processor.parse_chubproject(config_file)
...
def test_process_chubproject_calls_parse_and_build(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(build_options_processor, "parse_chubproject", mock_parse)
    monkeypatch.setattr(build_options_processor, "build_chub", mock_build)

    build_options_processor.process_chubproject(config_file)
...
def test_process_options_version_flag_with_installed_package(monkeypatch, capsys):
    ...
    monkeypatch.setattr(build_options_processor, "get_version", mock_get_version)

    build_options_processor.process_options(args)
...
def test_process_options_saves_chubproject(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ChubProject, "from_cli_args", mock_from_cli)
    monkeypatch.setattr(ChubProject, "save_file", mock_save)
    monkeypatch.setattr(build_options_processor, "build_chub", mock_build)
...
def test_process_options_chubproject_path_expansion(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(build_options_processor, "parse_chubproject", track_parse)
    monkeypatch.setattr(ChubProject, "override_from_cli_args", mock_override)
    monkeypatch.setattr(build_options_processor, "build_chub", mock_build)
...
def test_process_options_chubproject_save_path_expansion(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ChubProject, "from_cli_args", mock_from_cli)
    monkeypatch.setattr(ChubProject, "save_file", track_save)
    monkeypatch.setattr(build_options_processor, "build_chub", mock_build)
